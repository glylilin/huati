  ...
  Formatter = (function(_super) {
    ...
    function Formatter() {
      ...
      this._allowedAttributes = {
        img: ['src', 'alt', 'width','info','height', 'data-image-src', 'data-image-size', 'data-image-name', 'data-non-image'],
        a: ['href', 'target'],
        font: ['color'],
        pre: ['data-lang', 'class'],
        p: ['data-indent'],
  ...
  Toolbar = (function(_super) {
    ...
    Toolbar.prototype._init = function() {
      ...
      if (!$.isArray(this.opts.toolbar)) {
        this.opts.toolbar = ['bold', 'italic', 'underline', 'strikethrough', '|', 'ol', 'ul', 'blockquote', 'code', '|', 'indent', 'outdent','image'];
      }
      this._render();
  ...
  ImageButton = (function(_super) {
    ...
    function ImageButton(editor) {
      ...
      this.editor.body.on('click', 'img:not([data-non-image])', function(e) {
        ...
        var img_width = $img.attr('width') ? $img.attr('width') : cache.width;
        var img_height = $img.attr('height') ? $img.attr('height') : cache.height;
        $("#image-width").val(img_width);
        $("#image-height").val(img_height);
    ...
    ImageButton.prototype._initUploader = function() {
      ...
      this.editor.uploader.on('uploadsuccess', function(e, file, result) {
        ...
        if (result.isSuccessful === false) {
          ...
          $img.attr('id',result.data.id);
          _this.popover.widthEl.val(result.data.width);
          _this.popover.heightEl.val(result.data.height);
          $img.attr('info',result.data.id+"_"+result.data.width+'_'+result.data.height);
        }
...
ImagePopover.prototype._resizeImg = function(inputEl, onlySetVal) {
    ...
    var real_width=this.target.attr('width');
    var real_height = this.target.attr('height');
    ...
    if (inputEl.is(this.widthEl)) {
      width = value;
      height = parseInt(parseInt(real_height * value) / real_width);
      this.heightEl.val(height);
    } else {
      height = value;
      width =  parseInt(parseInt(real_width * value) / real_height);
    ...
    if (!onlySetVal) {
      ...
      if(this.target.attr("id")){
        this.target.attr("info",this.target.attr("id")+"_"+width+"_"+height);
      }
