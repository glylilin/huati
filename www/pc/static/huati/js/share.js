...
$(function(){
	$('.share_topic').click(function(e){
		if($(this).hasClass("isOIP")){
			...
			config._content=$(parents).find('.topic_brief_img').text() ? $(parents).find('.topic_brief_img').text() : $(parents).find('.topic_brief').text() ;
			...
			config._url = $(parents).find('.topic_list_head').find("a").eq(0).attr("href") ? "http://www.guest.com:8088"+$(parents).find('.topic_list_head').find("a").eq(0).attr("href") : document.URL;
